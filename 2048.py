...
class Play_2048(Tk):
    ...
    def show_number(self, row, column, a, b, c, d, num):
        ...
        color = {
            '2': '#776e65', '4': '#f9f6f2', '8': '#f9f6f2', '16': '#f9f6f2', '32': '#f9f6f2',
            ...
            for j in range(0, 4):
                ...
                for i in range(0, 4):
                    if self.game_board[i][j] == 0:
                        ...
                        if i + 1 <= 3 and self.game_board[i + 1][j] == self.game_board[i][j]:
                            self.game_board[i][j] *= 2
                            self.score += self.game_board[i][j]
                            self.game_board[i + 1][j] = 0
                        elif i + 2 <= 3 and self.game_board[i + 1][j] == 0 and self.game_board[i + 2][j] == self.game_board[i][j]:
                            self.game_board[i][j] *= 2
                            self.score += self.game_board[i][j]
                            self.game_board[i + 2][j] = 0
            ...
            for i in range(0, 4):
                ...
                for j in range(0, 4):
                    if self.game_board[i][j] == 0:
                        ...
                        if j + 1 <= 3 and self.game_board[i][j + 1] == self.game_board[i][j]:
                            self.game_board[i][j] *= 2
                            self.score += self.game_board[i][j]
                            self.game_board[i][j + 1] = 0
                        elif j + 2 <= 3 and self.game_board[i][j + 1] == 0 and self.game_board[i][j + 2] == self.game_board[i][j]:
                            self.game_board[i][j] *= 2
                            self.score += self.game_board[i][j]
                            self.game_board[i][j + 2] = 0
    ...
    # ==== check if all game blocks are full
    def full(self):
    ...
    def game_over(self):
        ...
        self.canvas.create_text(205, 205, font=("Arial", 40), fill="red", text="Game Over")

# ==== call the main class to run the program
Play_2048().mainloop()
